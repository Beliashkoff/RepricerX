    ...
    <div className="min-h-screen bg-[#f7f8fa] flex items-center justify-center p-4">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-[#111] mt-6 mb-1">Вход в кабинет</h1>
          <p className="text-sm text-[#666]">Управляйте ценами на маркетплейсах</p>
        </div>

        <div className="bg-white rounded-3xl border border-[#e6e6e6] p-8 shadow-sm">
          <div className="flex flex-col gap-3 mb-6">
            <SocialButton icon={<VKIcon />} label="Войти через VK ID" onClick={() => startOAuth('vk')} variant="vk" />
            <SocialButton icon={<YandexIcon />} label="Войти через Яндекс ID" onClick={() => startOAuth('yandex')} variant="yandex" />
          </div>
          ...
          <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-4">
            ...
            <div>
              <div className="flex items-center justify-between gap-4">
                ...
                <Link to="/forgot-password" className="text-xs font-medium text-[#111] hover:underline">
                  Забыли пароль?
                </Link>
              </div>
        ...
        <p className="text-center text-sm text-[#666] mt-6">
          Нет аккаунта?{' '}
          <Link to="/register" className="text-[#111] font-medium hover:underline">
            Зарегистрироваться
          </Link>
        </p>
      </div>
