...
func Load() (*Config, error) {
	...
	// В продакшене SMTP обязателен
	if cfg.Environment == "prod" && cfg.MailerMode == "smtp" {
		if cfg.SMTPHost == "" || cfg.SMTPUser == "" || cfg.SMTPPassword == "" {
			return nil, fmt.Errorf("SMTP_HOST, SMTP_USER, SMTP_PASSWORD обязательны в prod при MAILER_MODE=smtp")
		}
	}
